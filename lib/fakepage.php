...
class FakePage
{
	 ...
	 * @var string
	 */
	var $content = 'Assign $instance->content to what you want to display';

	/**
	 * Force the Injection of the fake page
	 * @var boolean
	 */
	...
	function detectPost($posts){
		 ...
		 */
		if (strtolower($wp->request) == strtolower($this->page_slug) || $wp->query_vars['page_id'] == $this->page_slug || $this->force_injection){
			//Add the fake post
			$posts=NULL;
